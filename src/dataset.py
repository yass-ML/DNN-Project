...
Uses P identities × K samples per identity per batch.
"""
...
class FaceDataset(Dataset):
    ...
    def _scan_directory(self):
        ...
        print(f"Loaded {len(self.samples)} samples from {len(self.identity_to_indices)} identities")
...
class IdentitySampler(Sampler):
    ...
    def __init__(self, dataset: FaceDataset, p_identities: int = 32,
                 k_samples: int = 4, num_iterations: Optional[int] = None):
        ...
        self.valid_identities = [
            label for label, indices in dataset.identity_to_indices.items()
        ...
        print(f"IdentitySampler: {len(self.valid_identities)} valid identities, "
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_iterations):
            ...
            selected_identities = random.sample(self.valid_identities, self.p_identities)
...
class BalancedBatchSampler(Sampler):
    """
    Alternative batch sampler that ensures balanced classes.
    
    Similar to IdentitySampler but samples without replacement within an epoch.
    """
    ...
    def __init__(self, dataset: FaceDataset, p_identities: int = 32,
        ...
        self.p_identities = p_identities
        self.k_samples = k_samples
        self.batch_size = p_identities * k_samples
        ...
        self.flat_indices = []
        for label, indices in self.identity_to_indices.items():
            for idx in indices:
                self.flat_indices.append((idx, label))
    
    def __iter__(self):
        # Shuffle identities
        identity_pool = self.identities.copy()
        random.shuffle(identity_pool)
        
        # Create index pools per identity
        index_pools = {
            label: indices.copy()
            for label, indices in self.identity_to_indices.items()
        }
        for pool in index_pools.values():
            random.shuffle(pool)
        
        while len(identity_pool) >= self.p_identities:
            batch = []
            
            # Select P identities
            selected = identity_pool[:self.p_identities]
            identity_pool = identity_pool[self.p_identities:]
            
            for identity in selected:
                pool = index_pools[identity]
                
                # Take K samples
                if len(pool) >= self.k_samples:
                    samples = pool[:self.k_samples]
                    index_pools[identity] = pool[self.k_samples:]
                else:
                    # Refill pool if exhausted
                    pool = self.identity_to_indices[identity].copy()
                    random.shuffle(pool)
                    samples = pool[:self.k_samples]
                    index_pools[identity] = pool[self.k_samples:]
            ...
            # Refill identity pool if getting low
            if len(identity_pool) < self.p_identities:
                identity_pool = self.identities.copy()
                random.shuffle(identity_pool)
    
    def __len__(self):
        return len(self.flat_indices) // self.batch_size
...
def get_dataloader(root: str, mode: str = 'train', 
                   p_identities: int = 32, k_samples: int = 4,
                   num_workers: int = 4, 
                   num_iterations: Optional[int] = None) -> DataLoader:
    """
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        loader = get_dataloader(
            str(root), mode='train',
            p_identities=4, k_samples=3,
            num_workers=0, num_iterations=5
        )
