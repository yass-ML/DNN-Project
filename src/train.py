...
def parse_args():
    ...
    parser.add_argument(
        "--lr_decay_factor", type=float, default=0.1, help="Learning rate decay factor"
    )
    parser.add_argument(
        "--weight_decay",
        type=float,
...
def train_epoch(model, dataloader, loss_fn, optimizer, device, epoch, writer, args):
    """Train for one epoch."""
    model.train()
    ...
    for batch_idx, (images, labels) in enumerate(pbar):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            if "fraction_semihard" in stats:
                writer.add_scalar(
                    "train/fraction_semihard", stats["fraction_semihard"], global_step
                )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_stats = train_epoch(
            model, train_loader, loss_fn, optimizer, device, epoch, writer, args
        )
        ...
        print(
            f"  Triplets: {train_stats['total_triplets']}, "
