...
class GaussianModel(nn.Module):
    ...
    def create_from_pcd(self, pcd: PointCloud, spatial_lr_scale: float = 1.0):
        ...
        colors = torch.tensor(pcd.colors, dtype=torch.float32) / 255.0  # Normalize to [0, 1]
        ...
        # Initialize gradient tracking for densification
    ...
    def add_densification_stats(self, viewspace_point_tensor: torch.Tensor, radii: torch.Tensor):
        """
        ...
        if viewspace_point_tensor.grad is not None:
            ...
            self.xyz_gradient_accum += torch.norm(grad, dim=-1, keepdim=True)
            self.denom += 1
    ...
    def _concat_params(self, xyz, features_dc, features_rest, scaling, rotation, opacity):
    ...
    def get_optimizer_param_groups(
        ...
        """
        return [
            {"params": [self._xyz], "lr": position_lr * self.spatial_lr_scale, "name": "xyz"},
