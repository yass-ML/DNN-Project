...
from src.dataset_optimized import get_optimized_dataloader
from src.loss_optimized import get_optimized_loss
...
def parse_args():
    ...
    # Data arguments
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Path to training data directory"
    )
    parser.add_argument(
        "--val_dir", type=str, default=None, help="Path to validation data directory"
    )
    ...
    parser.add_argument(
        "--pretrained", type=str, default=None, help="Path to pretrained weights"
    )
    ...
    parser.add_argument(
        "--p_identities", type=int, default=32, help="Number of identities per batch"
    )
    parser.add_argument(
        "--k_samples", type=int, default=4, help="Number of samples per identity"
    )
    parser.add_argument(
        "--lr", type=float, default=1e-3, help="Initial learning rate (lower for AdamW)"
    )
    parser.add_argument(
        "--weight_decay",
        type=float,
    ...
    # Loss arguments
    parser.add_argument(
        "--margin", type=float, default=0.2, help="Triplet loss margin (default: 0.2)"
    )
    parser.add_argument(
        "--loss_type",
        type=str,
    ...
    parser.add_argument(
        "--no_amp", action="store_false", dest="amp", help="Disable AMP"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--iterations_per_epoch", type=int, default=1000, help="Iterations per epoch"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    parser.add_argument(
        "--log_interval", type=int, default=50, help="Log every N iterations"
    )
    parser.add_argument(
...
def train_epoch(
    ...
    for batch_idx, (images, labels) in enumerate(pbar):
        ...
        if batch_idx % args.log_interval == 0:
            writer.add_scalar(
                "train/loss", loss.item() * args.gradient_accumulation, global_step
            )
            writer.add_scalar("train/lr", scheduler.get_last_lr()[0], global_step)

            if "num_triplets" in stats:
                writer.add_scalar(
                    "train/num_triplets", stats["num_triplets"], global_step
                )
...
def main():
    ...
    model = InceptionResNetV1(
        embedding_dim=args.embedding_dim, pretrained=args.pretrained
    )
    ...
    print(
        f"Batch size: {args.p_identities * args.k_samples} "
        f"({args.p_identities} identities × {args.k_samples} samples)"
    )
    if args.gradient_accumulation > 1:
        print(
            f"Effective batch size: {args.p_identities * args.k_samples * args.gradient_accumulation}"
        )
    print(f"{'=' * 50}\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"  Time: {epoch_time:.1f}s ({len(train_loader) / epoch_time:.1f} batches/sec)"
        )
