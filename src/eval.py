...
def load_model(model_path: str, embedding_dim: int, device: torch.device) -> InceptionResNetV1:
...
@torch.no_grad()
def compute_embeddings(model: InceptionResNetV1, dataloader: DataLoader, 
...
@torch.no_grad()
def compute_pair_embeddings(model: InceptionResNetV1, dataloader: DataLoader,
...
def find_optimal_threshold(distances: np.ndarray, labels: np.ndarray,
...
def evaluate_with_cross_validation(distances: np.ndarray, labels: np.ndarray,
                                   n_folds: int = 10) -> dict:
...
def evaluate_verification(model: InceptionResNetV1, dataloader: DataLoader,
                          device: torch.device, n_folds: int = 10) -> dict:
    ...
    print(f"  Accuracy: {cv_results['accuracy']*100:.2f}% ± {cv_results['accuracy_std']*100:.2f}%")
    print(f"  Threshold: {cv_results['threshold']:.4f} ± {cv_results['threshold_std']:.4f}")
...
def evaluate_identification(model: InceptionResNetV1, gallery_loader: DataLoader,
                            probe_loader: DataLoader, device: torch.device) -> dict:
...
def main():
    ...
    if args.pairs_file:
        ...
        results = {'num_embeddings': len(embeddings), 'num_identities': len(np.unique(labels))}
