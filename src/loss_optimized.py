...
class EfficientBatchHardLoss(nn.Module):
    ...
    def forward(
        ...
        valid_triplets = (hardest_pos_dist > -float("inf")) & (
        ...
        stats = {
            ...
            "num_hard": num_hard,
            "avg_pos_dist": hardest_pos_dist[valid_triplets].mean().item()
            if num_valid > 0
            else 0,
            "avg_neg_dist": hardest_neg_dist[valid_triplets].mean().item()
            if num_valid > 0
            else 0,
        }
    ...
    def forward(
        ...
        for anchor_idx in range(batch_size):
            ...
            for pos_idx in pos_indices:
                ...
                semi_hard_mask = (neg_distances > d_ap) & (
                    neg_distances < d_ap + self.margin
                )
...
class HybridTripletLoss(nn.Module):
    ...
    def __init__(
        self, margin: float = 0.2, squared: bool = True, soft_margin: bool = False
    ):
        super().__init__()
        self.margin = margin
        self.squared = squared
    ...
    def forward(
        ...
        stats = {
            ...
            "num_hard": int((triplet_loss[valid] > 0).sum().item())
            if valid.sum() > 0
            else 0,
            "avg_pos_dist": hardest_pos_dist[valid].mean().item()
            if valid.sum() > 0
            else 0,
            "avg_neg_dist": hardest_neg_dist[valid].mean().item()
            if valid.sum() > 0
            else 0,
        }
...
def get_optimized_loss(
    loss_type: str = "hybrid", margin: float = 0.2, squared: bool = True
) -> nn.Module:
    """
