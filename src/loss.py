    ...
    def forward(
        ...
        if self.soft:
            triplet_loss = torch.log1p(
                torch.exp(anchor_positive_dist - anchor_negative_dist)
            )
        else:
        ...
        for anchor_idx in range(batch_size):
            ...
            for pos_idx in positive_indices:
                ...
                if len(semi_hard_negatives) > 0:
                    ...
                    semi_hard_negatives[hardest_semi_hard_idx]
...
class SemiHardTripletLoss(nn.Module):
    ...
    def forward(
        ...
        get_anchor_negative_triplet_mask(labels)

        # Shape: (N, N) - all anchor-positive distances
        pairwise_dist * ap_mask.float()
        ...
        semi_hard_mask = (an_dist_3d > ap_dist_3d) & (
            an_dist_3d < ap_dist_3d + self.margin
        )
...
def get_triplet_loss(
    loss_type: str = "semihard", margin: float = 0.2, squared: bool = True
) -> nn.Module:
    """
